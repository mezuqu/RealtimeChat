...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	if ([type isEqualToString:CHAT_PRIVATE])
	{
		DBUser *dbuser = [self fetchOtherUser];
		labelTitle.text = dbuser.fullname;
	...
	if ([type isEqualToString:CHAT_GROUP])
	{
		DBGroup *dbgroup = [self fetchGroup];
		labelTitle.text = dbgroup.name;
		NSInteger count = [[dbgroup.members componentsSeparatedByString:@","] count];
		labelDetails.text = [NSString stringWithFormat:@"%ld members", (long) count];
	}
}

//-------------------------------------------------------------------------------------------------------------------------------------------------
- (DBUser *)fetchOtherUser
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	NSString *userId = [self otherUserId];
	NSPredicate *predicate = [NSPredicate predicateWithFormat:@"objectId == %@", userId];
	return [[DBUser objectsWithPredicate:predicate] firstObject];
}
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	...
	return [[DBGroup objectsWithPredicate:predicate] firstObject];
}
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	...
	{
		if ([userId isEqualToString:[FUser currentId]] == NO) return userId;
	}
	return nil;
}
