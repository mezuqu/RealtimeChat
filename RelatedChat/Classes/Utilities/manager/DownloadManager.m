...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	...
	if ([File exist:loading])
	{
		...
		if (time - check < DOWNLOAD_TIMEOUT) { if (completion != nil) completion(nil, [NSError description:@"Downloading." code:102], NO); return; }
	}
	//---------------------------------------------------------------------------------------------------------------------------------------------
		...
		if (error == nil)
		{
			...
			if ([File size:path] != 0)
			{
				if (md5 != nil)
				{
					...
					else [self failed:file error:[NSError description:@"MD5 checksum." code:103] completion:completion];
				}
				else [self succeed:file completion:completion];
			}
			else [self failed:file error:[NSError description:@"Zero lenght." code:104] completion:completion];
		}
